...
const adapt = (handler: any) => {
  return async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.error('[Adapter Error] Falla en la ejecución del controlador adaptado:', error);
      if (!res.headersSent) {
        res.status(500).json({
          error: 'Internal Server Error',
          message: 'Ocurrió un error inesperado al procesar la solicitud en el enrutador central.'
        });
      }
    }
  };
